 ...
 * @date 02-10-2023
 * @brief Classe generica Vector<T> que implementa uma lista linear sequencial de inteiros
 */
#ifndef VECTOR_H
#define VECTOR_H
#include <stdexcept>
...
public:
    /*
    Classe interna da Classe Vector, que implementa a logica de um interador
    */
    class interator{
        private:
            T *m_ptr{nullptr}; //ponteiro para um elemento do Vector<T>
        public:
            //Construtor customizado
            // Recebe um ponteiro e cria iterador apontando
            // para o endereço passado por parametro
            interator(T *ptr){
                m_ptr = ptr;
            }
            // Operador de desreferencia
            // Retorna uma referencia para o elemento
            // apontando pelo interator
            T& operator*(){
                return *m_ptr;
            }
            // Operador seta. Retorna o valor do
            // atributo privado pelo iterador, que é um ponteiro.
            T* operator->(){
                return m_ptr;
            }
            //Operador de indexação
            // Retorna uma referência para o elemento
            // Localizado n posições após a posição
            // atual do iterador
            T& operator[](long n){
                return *(m_ptr + n);
            }
            // Operador de pré-incremento.
            //Permite expressões do tipo ++it
            interator& operator++(){
                m_ptr++;
                return *this;
            }
    ...
    //----------------------------------------------------------
    // retorna um iterador para o inicio da lista
    iterator begin(){
        return iterador(m_vet);
    }
    // retorna um iterador para o final da lista
    iterator end(){
        return iterador(m_vet + m_size);
    }
    ...
    // Faz uma atribuicao entre vectors
    Vector& operator=(const Vector& v);

    // Compara dois vectors
    // Retorna 'true' se, e somente se, os vetores sao identicos
    // Dois vectors sao identicos se tem o mesmo tamanho 
